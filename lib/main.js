...
new Promise( (resolve, reject) => {
    ...
    var folder = data[0];
    var allVals = data[1];
    var cmdSetOutputDir = `OUTPUT_DIR="${folder}"`;
    console.log(cmdSetOutputDir);
    var cmdCreateDir = 'mkdir -p "$OUTPUT_DIR"';
    ...
    allVals.forEach( (v, i) => {
        v.then( (html) => {
            ...
            var filepath = "$OUTPUT_DIR/" + leftPad(i, allVals.length);
            var filename = "$OUTPUT_DIR/" + leftPad(i, allVals.length);
            textToSpeech(text, filepath, filename).then( () => {
